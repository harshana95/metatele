...
from models.OneStepDiffusion.onedataset_model import onedataset_model, encode_prompt, eps_to_mu
from pipelines.OneStepDiffusionPipeline import OneStepDiffusionPipeline
...
class twodataset_model(onedataset_model, TwoDatasetBasemodel):
    def __init__(self, opt, logger):
        super(twodataset_model, self).__init__(opt, logger)
    ...
    def forwardpass(self, lq1, lq2, prompt_embeds=None):
        if lq1.shape[1] == 1:
    ...
    def validate_step(self, batch, idx,lq_key,gt_key):
        ...
        caption_input = self.sample[lq_key + '_1_original'] if self.opt.val.patched else self.sample[lq_key + "_1"]
        ...
        adapter_input = None
        if self.use_adapter:
            adapter_lq1, adapter_lq2 = lq1, lq2
            if adapter_lq1.shape[1] == 1:
                adapter_lq1 = einops.repeat(adapter_lq1, 'b 1 h w -> b 3 h w')
            if adapter_lq2.shape[1] == 1:
                adapter_lq2 = einops.repeat(adapter_lq2, 'b 1 h w -> b 3 h w')
            adapter_input = self.adapter_preprocess(adapter_lq1, adapter_lq2).cpu().numpy()*0.5+0.5
            if adapter_input.shape[1] == 6:
                adapter_input = einops.rearrange(adapter_input, 'n (c1 c2) h w -> n c1 (h c2) w', c1=3, c2=2)
            elif adapter_input.shape[1] == 1:
