...
def log_prompt(args, prompt, name, step):
    prompts_dir = os.path.join(args.path.experiments_root, 'prompts')
    os.makedirs(prompts_dir, exist_ok=True)
    filename = f"{name}_{step // 1000:04d}k.txt"
    with open(os.path.join(prompts_dir, filename), 'w', encoding='utf-8') as f:
        f.write(prompt if prompt else "")
